...
def main():
    ...
    ncbi.search(taxid)
    print(f"Found {ncbi.count} records. Fetching up to {limit}...")
    seqs,step=[],500
    for i in range(0,min(ncbi.count, limit), step):
        print(f"Fetching {i} - {i+min(step,limit-i)}...")
        for rec in ncbi.fetch(i,min(step,limit-i)):
            l=len(rec.seq)
            if minlen<=l<=maxlen:
                seqs.append((rec.id,l,rec.description))
    if not seqs:
        print("No records in specified length range.");return
